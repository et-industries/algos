...
pub fn run_test() {
    ...
    let tp = Tape::<f32, ScalarOps>::new();
    // weights
    let mut ws = vec![tp.var(0.4), tp.var(0.8), tp.var(0.1)];
    // inputs
    let inp = vec![tp.var(2.0), tp.var(4.0), tp.var(6.0)];
    // label
    let label = tp.var(0.4);

    for _ in 0..10000 {
        println!("");
        println!("ws: {:?}", ws.iter().map(|x| x.data).collect::<Vec<f32>>());
        ...
        // calculate error
        let error = (&label - &res).powf(&tp.var(2.0));
        // backward pass
        error.reverse();
        ...
        println!(
            "grads: {:?}",
            ws.iter().map(|x| x.grad()).collect::<Vec<f32>>()
        );

        ws = ws
            .iter()
            .map(|x| tp.var(x.data - x.grad() * lr))
            .collect::<Vec<Var<f32, ScalarOps>>>();
    }
}
